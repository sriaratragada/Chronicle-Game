...
const DialogueOverlay = memo(function DialogueOverlay() {
...
const DeathScreen = memo(function DeathScreen({ onRestart }: { onRestart: () => void }) {
  const [showConfirm, setShowConfirm] = useState(false);
  const tick = useGameStore(s => s.tick);
  const season = useGameStore(s => s.season);
  const lastEntry = useGameStore(s => {
    const c = s.chronicle;
          ...
          >
            ...
            >
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={() => { setShowConfirm(false); onRestart(); }}
